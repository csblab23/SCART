...
def _run_infercna(
    ...
    if len(common) < 200:
        raise ValueError("Too few common genes for inferCNA")

    q_idx = np.where(np.isin(adata_query.var_names, common))[0]
    r_idx = np.where(np.isin(adata_ref.var_names, common))[0]
    ...
    ro.r(f"""
        ...
        cna <- infercna(
            m = r_mat,
            refCells = list(normal_ref = ref_cells),
            n = {n},
            noise = {noise},
            isLog = TRUE
        )
    ...
    if modes is None:
        return pd.Series("not.defined", index=q_barcodes)

    label_map = {}
    for name in modes.names:
        label = "malignant" if "malignant" in name.lower() else "non-malignant"
        for bc in list(modes.rx2(name)):
            label_map[bc] = label
