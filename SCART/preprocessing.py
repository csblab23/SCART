...
def build_tumor_healthy_combined(
    ...
    # CLAUDE EDIT — Cell_Type column, for the UMAP plot's third coloring.
    # Healthy side: whichever native cell-type annotation column the
    # Tabula Sapiens reference actually has (checked in priority order —
    # exact naming varies by reference file version). This reads an
    # already-loaded small metadata column on adata_h (no new file I/O, no
    # new large array), so it carries essentially no memory cost.
    # Tumor side: a constant label — every cell here is already restricted
    # to malignant epithelial cells by this point in the pipeline (Step 5
    # of preprocessing.py), so there is nothing further to distinguish.
    _CELLTYPE_CANDIDATES = ["cell_ontology_class", "free_annotation", "cell_type", "celltype"]
    _ct_col = next((c for c in _CELLTYPE_CANDIDATES if c in adata_h.obs.columns), None)
    if _ct_col:
        adata_h.obs["Cell_Type"] = adata_h.obs[_ct_col].astype(str)
        logger.info(f"[healthy] Cell_Type sourced from column '{_ct_col}'")
    else:
        adata_h.obs["Cell_Type"] = "Healthy (cell type unavailable)"
        logger.warning(
            f"[healthy] No cell-type column found (checked {_CELLTYPE_CANDIDATES}) — "
            "Cell_Type panel will show a single category for all healthy cells."
        )
    adata_t.obs["Cell_Type"] = "Malignant epithelial cell"
...
def _scatter(ax, coords, labels, title, legend=True):
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    if legend and len(labels.categories) <= 30:
...
def plot_umap_before_after(adata_hvg, save_dir, sample_name="cancer_composition"):
    ...
    # CLAUDE EDIT — layout: rows = coloring dimension (Data_Type, batch,
    # Cell_Type), columns = unintegrated (LEFT) vs. Harmony-integrated
    # (RIGHT), per request. Cell_Type row only added if the column exists
    # (build_tumor_healthy_combined always sets it now, but this keeps the
    # function itself safe/standalone either way).
    color_rows = [("Data_Type", "Data_Type"), ("batch", "batch")]
    if "Cell_Type" in adata_hvg.obs.columns:
        color_rows.append(("Cell_Type", "Cell_Type"))
    ...
    for row_i, (obs_col, label) in enumerate(color_rows):
        _scatter(axes[row_i, 0], umap_unintegrated, adata_hvg.obs[obs_col],
                 f"Unintegrated — {label}")
        _scatter(axes[row_i, 1], umap_harmony, adata_hvg.obs[obs_col],
                 f"Harmony-integrated — {label}")
