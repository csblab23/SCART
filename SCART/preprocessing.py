...
def run_preprocessing_pipeline(
    ...
    # CLAUDE EDIT — upgrade to GENUINE full gene space via Module 2's own
    # sidecar file. adata_full's layers['counts'] is HVG-space (per this
    # module's own docstring above, and popv_annotation.py's design) — the
    # true full common-gene-space matrix lives ONLY in the separate sidecar
    # h5ad Module 2 writes specifically for this (uns['full_counts_h5ad_path']).
    # Without this upgrade, adata_mal_fullgene would silently inherit
    # whatever gene space Module 2's Process_Query happened to produce —
    # correct for datasets where that never gets reduced, but not something
    # to rely on implicitly. Falls back to the HVG/common-gene snapshot
    # above if the sidecar path is missing, the file can't be found (e.g.
    # working directory changed since Module 2 ran), or barcodes don't
    # fully align (e.g. manual_annotation_col / skip_popv path, which never
    # writes a sidecar at all).
    ...
    if _fullgene_sidecar_path and os.path.exists(_fullgene_sidecar_path):
        try:
            ...
            if len(_common_barcodes) == adata_mal_fullgene.n_obs:
                _sidecar_mal = _sidecar[_mal_barcodes].copy()
                # Keep OUR malignant-calling obs columns (scevan_prediction,
                # final_malignant, etc.) — the sidecar only has Module 2's
                # own query-level annotation columns, not these.
                _sidecar_mal.obs = adata_mal_fullgene.obs.loc[_mal_barcodes].copy()
                if "counts" not in _sidecar_mal.layers:
                    _sidecar_mal.layers["counts"] = _sidecar_mal.X.copy()
                adata_mal_fullgene = _sidecar_mal
                print(
                    f"  adata_mal_fullgene upgraded to genuine full gene space "
                    f"via sidecar: {adata_mal_fullgene.n_vars} genes "
                    f"(was {adata_mal.n_vars} in Module 2's own gene space)"
                )
            else:
                print(
                    f"  WARNING: full-gene sidecar barcode overlap incomplete "
                    f"({len(_common_barcodes)}/{adata_mal_fullgene.n_obs} malignant "
            ...
            print(
                ...
                f"keeping Module 2 gene-space snapshot for adata_mal_fullgene."
            )
    else:
        print(
            "  No full-gene sidecar found (adata.uns['full_counts_h5ad_path']) — "
            "adata_mal_fullgene remains in Module 2's own gene space. This is "
            "expected if PopV was skipped (manual_annotation_col) or the "
            "sidecar file has moved since Module 2 ran."
        )
