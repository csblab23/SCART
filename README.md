![SCART — Single-Cell CAR-T Target Discovery](SCART/external/scart_logo.png)

**SCART** is an end-to-end computational pipeline for identifying tumour-specific surface protein targets for CAR-T cell therapy from single-cell RNA-seq data. Starting from **RAW** GEO accession IDs or user-provided h5ad files with **RAW** data, SCART automates cell-type annotation, malignant cell identification, surfaceome differential expression, and logic-gate gene combination scoring to rank One and Two gene combination candidate CAR-T targets by efficacy and safety. Detailed information and tutorial is at https://csblab23.github.io/SCART/
...
adata = popv_annotation.auto_run_popv(
    nsamples       = 300, # you can choose no. of sample for you data
